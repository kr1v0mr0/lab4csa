...
Вариант `asm | stack | neum | mc | tick | binary | stream | port | cstr | prob1 | vector` не содержит требования `intr`, поэтому аппаратная модель прерываний **не реализована**. В симуляторе нет IRQ-линий, режима ядра, автоматического сохранения контекста и микропрограммы входа в обработчик.

Чтобы показать, где это находилось бы в выбранной архитектуре, фиксируется концептуальная схема:

| Область | Возможное назначение при добавлении прерываний |
|---------|------------------------------------------------|
| `memory[0]` | Стартовая команда или `JMP program_start`. |
| `memory[1..k]` | Таблица векторов: адреса обработчиков `irq0`, `irq1`, ... |
| `memory[program_start..]` | Обычный код и статические данные. |
| `memory[handler_addr..]` | Код обработчиков. |
| Стек | Сохраненные `PC`, флаги `Z/N`, рабочие значения обработчика. |
| `MICRO_ROM_WORDS` | Возможная микропрограмма `INT_ENTER`: сохранить контекст, прочитать вектор, загрузить `PC`. |

Прерывания не являются частью выбранного варианта; место в памяти и связь с микрокодом описаны концептуально.
...
DataPath
      ...
      PC mux: PC+1 / IR.arg / DR / TMP
        ...
        +----------------+          +----------------+
        | Input ports    |--------->|       DR       |---------> Output ports
        | stream, port 0 |    IN    +----------------+   OUT    | port 1 |
        +----------------+                                      +--------+
...
**Место для скриншота схемы Control Unit:**

![Control Unit](docs/img/control_unit.png)

схема CU не дублирует все линии к каждому регистру DataPath. Блок `Datapath control / signal bus` обозначает группу управляющих сигналов: `LATCH_*`, `READ`, `WRITE`, `ALU_*`, `IN`, `OUT`, `V_*`.
...
```text
                       ...
                       +-----------------------+
                                  ...
                                  v
   +----------------------------------------------------------------+
   | Datapath control / signal bus: latch_*, mem_*, alu_*, in/out   |
   | stack_*, vector_*                                              |
   +----------------------------------------------------------------+
                                  |
                                  v
...
## 13. Ограничения и честные упрощения

- Сообщения об ошибках минимальные.
- Макросы позиционные, без именованных параметров.
- `.section` является логическим маркером, отдельного загрузчика и линковщика нет.
- Аппаратные прерывания не реализованы, так как `intr` отсутствует в варианте.
- Микрослова представлены кортежами имен сигналов, а не сжатым битовым форматом.
- Непосредственное поле инструкции - 20 бит; большие значения нужно строить несколькими инструкциями или хранить в памяти.
- Слово памяти 32-битное по формату машинного кода, но арифметика симулятора использует Python `int`, что упрощает переполнение.
